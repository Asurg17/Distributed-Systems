...
func (rf *Raft) Snapshot(snapshot []byte, commandIndex int) {
	rf.mu.Lock()
	defer rf.mu.Unlock()
	...
	if commandIndex < firstLogIndex || commandIndex > lastLogIndex { // if invalid index
		return
	}

	rf.discardPrecedeLogEntries(commandIndex, rf.log[commandIndex-firstLogIndex].Term)

	if len(rf.log) == 0 {
		rf.log = rf.log[commandIndex:]
	} else {
		rf.log = rf.log[(commandIndex - rf.log[0].Index):]
	}

	s := new(bytes.Buffer)
	n := labgob.NewEncoder(s)
	if len(rf.log) == 0 {
		n.Encode(0)
		n.Encode(0)
	} else {
		n.Encode(rf.log[0].Index)
		n.Encode(rf.log[0].Term)
	}
	newSnapshot := append(s.Bytes(), snapshot...)

	rf.persister.SaveStateAndSnapshot(rf.getRaftState(), newSnapshot)
}
...
func (rf *Raft) InstallSnapshot(args *InstallSnapshotArgs, reply *InstallSnapshotReply) {
	...
	if args.LastIncludedIndex > rf.commitIndex {
		rf.discardPrecedeLogEntries(args.LastIncludedIndex, args.LastIncludedTerm)
...
func (rf *Raft) readSnapshot(data []byte) {
	...
	if d.Decode(&LastIncludedIndex) != nil || d.Decode(&LastIncludedTerm) != nil {
		fmt.Printf("Error: data not found!")
	} else {

		rf.commitIndex = LastIncludedIndex
		rf.lastApplied = LastIncludedIndex

		rf.discardPrecedeLogEntries(LastIncludedIndex, LastIncludedTerm)

		go func() {
			rf.chanApply <- ApplyMsg{Snapshot: data}
		}()
...
func (rf *Raft) discardPrecedeLogEntries(lastIncludedIndex int, lastIncludedTerm int) {
	initialLogEntry := LogEntry{Term: lastIncludedTerm, Index: lastIncludedIndex}
	newLogEntries := make([]LogEntry, 0)
	newLogEntries = append(newLogEntries, initialLogEntry)

	for i := len(rf.log) - 1; i >= 0; i-- {
		if rf.log[i].Term == lastIncludedTerm && rf.log[i].Index == lastIncludedIndex {
